...
% 6) Set the path for 'outputDir' to where output files should be saved.
...
%set path to outputDir for location to save all output files.
outputDir ='/Users/x13j698/Library/CloudStorage/OneDrive-MontanaStateUniversity/MSU/MATLAB/Supply-Demand'


%CEV needs to run at beginning
  PATH_SEP = pathsep();
    wget_path = '/opt/homebrew/bin'; % Or wherever wget is installed
    currPath = getenv('PATH');
    setenv('PATH', [currPath, PATH_SEP, wget_path]);

AmPdataPath='/Users/x13j698/Library/CloudStorage/OneDrive-MontanaStateUniversity/MSU/MATLAB'
%AmPtoolPath='/Users/x13j698/Library/CloudStorage/OneDrive-MontanaStateUniversity/MSU/MATLAB/AmPtool'
%DEBtool_MPath='/Users/x13j698/Library/CloudStorage/OneDrive-MontanaStateUniversity/MSU/MATLAB/DEBtool_M'
...
if ~exist('fig','var')
  ...
  legend_custom = { ... % colors from legend_vert
...
PMR.method = ['Loco.land','Loco.water','Loco.fly', 'Loco.flyburst', 'DEEModel', 'Turpor.arousal', 'Digest', 'Righting','Cold','Indirect','Check'];
...
ave = { ...  % Aves ; m(g) Tb(C) BMR(ml O2/min) PMR (ml O2/min), PMRmethod; %x means not in AmP
    ...
    % Anseriformes
    [275    4.12   NaN  23.22]  'Indirect', 'Kirk1983',     'Dendrocygna_autumnalis' % PMR 672 kJ/d; 20.1 kJ/l O2 omit MMR quantified as dietary energy intake
   [944.1 39.7 12.23  61.51]  'Cold', 'HindBaud1993', 'Anas_castanea' %x
    [190    41.2   NaN  21.39]  'Indirect', 'Kirk1983',     'Aythya_affinis' % PMR 619 kJ/d; 20.1 kJ/l O2 omit MMR quantified as dietary energy intake
   ...
   [ 419   41.7  7.08  162.8]  'Indirect', 'Pear1964',     'Columba_livia' % PMR 112 kcal/h.kg (4.184 J/cal; 20.1 kJ/l O2); FAS 23; omit metabolism based on weight loss not oxygen consumption
    ...
    % Apodiformes
    [ 5.7   40.7  NaN   5] 'Loco.fly', 'BergHart1972', 'Chionomesa_fimbriata' % PMR 43 ml/h.g, omit BMR not measured; values and FAS of 14 reported are based on Lasi's BMR values for other species and MMR may not be max
    ...
    % Suliformes
  [1300   40.6   NaN  64.95]  'Indirect', 'Kirk1983',     'Phalacrocorax_pelagicus' % PMR 1880 kJ/d; 20.1 kJ/l O2 omit MMR quantified as dietary energy intake
    % Charadriiformes
   ...
   [63.4   41.0 1.409 16.906] 'Indirect', 'Putt1980',     'Calidris_ferruginea' % 1699 kJ/h; FAS 12; 20.1 kJ/l O2; discard for now metabolism based on activity and feeding observations not O2 concumption
    ...
    % Falconiformes
   [106    40.0   NaN  11.09]  'Indirect', 'Kirk1983',     'Falco_tinnunculus' % PMR 321 kJ/d; 20.1 kJ/l O2 omit MMR quantified as dietary energy intake
    % Piciformes
  ...
  [18.2   41.6   NaN   3.26]  'Indirect', 'Kirk1983',     'Delichon_urbicum' % PMR 94.5 kJ/d; 20.1 kJ/l O2 Hirundinidae omit MMR quantified as dietary energy intake
 ...
 [13.36   42.1   NaN   4.15] 'Cold', 'WierChap2007', 'Tachycineta_albilinea' % PMR 1.39 W; 20.1 kJ/l O2 Hirundinidae
   ...
   [30.0   41.6   NaN   3.80]  'Indirect',     'Kirk1983',     'Fringilla_coelebs' % PMR 110 kJ/d; 20.1 kJ/l O2 Fringillidae omit MMR quantified as dietary energy intake
   ...
   [32.4   41.6   NaN   4.46] 'Indirect',         'Kirk1983',     'Zonotrichia_leucophrys' % PMR 129 kJ/d; 20.1 kJ/l O2 Passerellidae omit MMR quantified as dietary energy intake
   [28.0   41.6   NaN   4.91]  'Indirect',         'Kirk1983',     'Zonotrichia_albicollis' % PMR 142 kJ/d; 20.1 kJ/l O2 Passerellidae omit MMR quantified as dietary energy intake
 ...
 mar = { ... % Marsupialia & Prototheria; m(g), Tb(C), BMR(ml O2/min), PMR (ml O2/min), PMR.method; %x means not in AmP
   ...
   % Marsupialia
    ...
    [   34   30   21.42  70.04]  'Loco.land', 'WithThom2000', 'Notoryctes_caurinus' % 0.63 2.06 mL O2/h.g Omit COT not actually PMR
  };
 ...
 pla = {... % Placentalia; m(g), Tb(C), BMR(ml O2/min), PMR (ml O2/min), PMR.method; %x means not in AmP
   ...
   [ 1458   36.7   NaN  154.8] 'Loco.land', 'TaylMalo1980', 'Genetta_tigrina' %x 2.58 ml O2/s
    [ 41000  39.0 311.2  14700] 'Indirect', 'ScanMill2014', 'Acinonyx_jubatus' % FMR=DEE 9006 kJ/d PMR 120 W/kg; 20.1 kJ/l O2 omit data based on DEE model
   [ 25000  38.0 117.5   2900] 'Indirect', 'GormMill1998', 'Canis_familiaris' % BMR 3.4 MJ/d canid AS 25; 20.1 kJ/l O2 omit data based on DEE model
  [450000  36.8   NaN   5782] 'Indirect', 'Kirk1983',     'Ursus_maritimus' % PMR 167360 kJ/d; 20.1 kJ/l O2 omit MMR quantified as dietary energy intake
   [    86  39.6   NaN   7.34] 'Indirect', 'Kirk1983',     'Mustela_nivalis' % PMR 224 kJ/d; 20.1 kJ/l O2 omit MMR quantified as dietary energy intake
   ...
   [635000  38.0   NaN   4957] 'Indirect', 'Kirk1983',     'Equus_ferus_caballus' % PMR 143470 kJ/d; 20.1 kJ/l O2. Reliable? PMR is less than for a cow omit MMR quantified as dietary energy intake
   % Cetartiodactyla
   [205000  39.0   NaN   2371] 'Indirect', 'Kirk1983',     'Sus_domesticus' % PMR 68620 kJ/d; 20.1 kJ/l O2 omit MMR quantified as dietary energy intake
 ...
 [5.318e6 36.0  6633  39635] 'Loco.water', 'WortWort2013', 'Orcinus_orca' % 8.0 47.8 MJ/h; 20.1 kJ/l O2 Omit PMR is estimates based on activity observations
   % Rodentia
...
for g = 1:length(groupNames)
    ...
    groupData = eval(groupName); % original cell array

    % Initialize table
    T = table( ...
        strings(size(groupData,1),1), ...
        strings(size(groupData,1),1), ...
        strings(size(groupData,1),1), ...
        zeros(size(groupData,1),1), ...
        zeros(size(groupData,1),1), ...
        zeros(size(groupData,1),1), ...
        zeros(size(groupData,1),1), ...
        false(size(groupData,1),1), ...
        'VariableNames', {'species','bibkey','PMR_method','mass_g','temp_C','SMR_mlO2_min','MMR_mlO2_min','inAmP'});

    for i = 1:size(groupData,1)
        ...
        T.mass_g(i) = entry(1);
        T.temp_C(i) = entry(2);
        T.SMR_mlO2_min(i) = entry(3);
        T.MMR_mlO2_min(i) = entry(4);
    ...
    for j = 1:size(T,1)
        filteredGroup{j,1} = [T.mass_g(j), T.temp_C(j), T.SMR_mlO2_min(j), T.MMR_mlO2_min(j)];
        filteredGroup{j,2} = T.PMR_method(j);
        filteredGroup{j,3} = T.bibkey(j);
        filteredGroup{j,4} = char(T.species(j)); % ensure char format
    end

    [cleanData, cleanGroup] = cleanGroupData(filteredGroup);

    groupTables.(groupName) = T;
    groupClean.(groupName) = cleanGroup;
    groupDataMatrix.(groupName) = cleanData;
end
...
for c=1:length(fig)
  ...
  switch fig(c)
    case 1 % ss_AS
     ...
     PMR_SMR_act = groupDataMatrix.act(:,4) ./ groupDataMatrix.act(:,3);
      ss_act = read_stat(groupClean.act(:,4),'s_s');
      %
     PMR_SMR_cho = groupDataMatrix.cho(:,4) ./ groupDataMatrix.cho(:,3);
      ss_cho = read_stat(groupClean.cho(:,4),'s_s');
      %
    PMR_SMR_amp = groupDataMatrix.amp(:,4) ./ groupDataMatrix.amp(:,3);
      ss_amp = read_stat(groupClean.amp(:,4),'s_s');
      %
     PMR_SMR_squ = groupDataMatrix.squ(:,4) ./ groupDataMatrix.squ(:,3);
      ss_squ = read_stat(groupClean.squ(:,4),'s_s');
      %
    PMR_SMR_ave = groupDataMatrix.ave(:,4) ./ groupDataMatrix.ave(:,3);
      ss_ave = read_stat(groupClean.ave(:,4),'s_s');
      %
     PMR_SMR_mar = groupDataMatrix.mar(:,4) ./ groupDataMatrix.mar(:,3);
      ss_mar = read_stat(groupClean.mar(:,4),'s_s');
      %
     PMR_SMR_pla = groupDataMatrix.pla(:,4) ./ groupDataMatrix.pla(:,3);
      ss_pla = read_stat(groupClean.pla(:,4),'s_s');
      ...
      h = datacursormode(Hfig); entries_txt = [groupClean.cho(:,4); groupClean.act(:,4); groupClean.amp(:,4); groupClean.squ(:,4); groupClean.ave(:,4); groupClean.mar(:,4); groupClean.pla(:,4)]; 
      ...
      saveas(gcf, fullfile(outputDir,'ss_PSMR.fig'))
      saveas(gcf, fullfile(outputDir,'ss_PSMR.png'))

    case 2  % predicted FMR_measured SMR   
      WD0 = pwd; % store original directory
      nm_cho = groupClean.cho(:,4); n_cho = length(nm_cho); FMR_cho = zeros(n_cho,1); 
     PMR_cho = groupDataMatrix.cho(:,4); SMR_cho = groupDataMatrix.cho(:,3); T_cho = groupDataMatrix.cho(:,2); W_cho = groupDataMatrix.cho(:,1); 
      for i=1:n_cho
        FMR_cho(i) = 15.55 * get_FMR(nm_cho{i}, W_cho(i), T_cho(i), 1); % ml O2/min
      end
      %
      nm_act = groupClean.act(:,4); n_act = length(nm_act); FMR_act = zeros(n_act,1); 
    PMR_act = groupDataMatrix.act(:,4); SMR_act = groupDataMatrix.act(:,3); T_act = groupDataMatrix.act(:,2); W_act = groupDataMatrix.act(:,1); 
      for i=1:n_act
        FMR_act(i) = 15.55 * get_FMR(nm_act{i}, W_act(i), T_act(i), 1); % ml O2/min
      end
      %
      nm_amp = groupClean.amp(:,4); n_amp = length(nm_amp); FMR_amp = zeros(n_amp,1); 
     PMR_amp = groupDataMatrix.amp(:,4); SMR_amp = groupDataMatrix.amp(:,3); T_amp = groupDataMatrix.amp(:,2); W_amp = groupDataMatrix.amp(:,1); 
      for i=1:n_amp
        FMR_amp(i) = 15.55 * get_FMR(nm_amp{i}, W_amp(i), T_amp(i), 1); % ml O2/min
      end
      %
      nm_squ = groupClean.squ(:,4); n_squ = length(nm_squ); FMR_squ = zeros(n_squ,1); 
      PMR_squ = groupDataMatrix.squ(:,4); SMR_squ = groupDataMatrix.squ(:,3); T_squ = groupDataMatrix.squ(:,2); W_squ = groupDataMatrix.squ(:,1); 
      for i=1:n_squ
        FMR_squ(i) = 15.55 * get_FMR(nm_squ{i}, W_squ(i), T_squ(i), 1); % ml O2/min
      end
      %
      nm_ave = groupClean.ave(:,4); n_ave = length(nm_ave); FMR_ave = zeros(n_ave,1);
    PMR_ave = groupDataMatrix.ave(:,4); SMR_ave = groupDataMatrix.ave(:,3); T_ave = groupDataMatrix.ave(:,2); W_ave = groupDataMatrix.ave(:,1); 
      for i=1:n_ave
        FMR_ave(i) = 15.55 * get_FMR(nm_ave{i}, W_ave(i), T_ave(i), 1); % ml O2/min
      end
      %
      nm_mar = groupClean.mar(:,4); n_mar = length(nm_mar); FMR_mar = zeros(n_mar,1);
        PMR_mar = groupDataMatrix.mar(:,4); SMR_mar = groupDataMatrix.mar(:,3); T_mar = groupDataMatrix.mar(:,2); W_mar = groupDataMatrix.mar(:,1); 
      for i=1:n_mar
        FMR_mar(i) = 15.55 * get_FMR(nm_mar{i}, W_mar(i), T_mar(i), 1); % ml O2/min
      end
      %
      nm_pla = groupClean.pla(:,4); n_pla = length(nm_pla); FMR_pla = zeros(n_pla,1);
      PMR_pla = groupDataMatrix.pla(:,4); SMR_pla = groupDataMatrix.pla(:,3); T_pla = groupDataMatrix.pla(:,2); W_pla = groupDataMatrix.pla(:,1); 
      for i=1:n_pla
      ...
      saveas(gcf,fullfile(outputDir,'FMR_SMR.fig'))
      saveas(gcf,fullfile(outputDir,'FMR_SMR.png'))

    case 3  % ss_PMR/FMR
      WD0 = pwd; % store original directory
      nm_cho = groupClean.cho(:,4); n_cho = length(nm_cho); FMR_cho = zeros(n_cho,1); ss_cho = read_stat(nm_cho, 's_s');
      PMR_cho = groupDataMatrix.cho(:,4); SMR_cho = groupDataMatrix.cho(:,3); T_cho = groupDataMatrix.cho(:,2); W_cho = groupDataMatrix.cho(:,1); 
      for i=1:n_cho
        FMR_cho(i) = 15.55 * get_FMR(nm_cho{i}, W_cho(i), T_cho(i), 1); % ml O2/min
      end
      %
      nm_act = groupClean.act(:,4); n_act = length(nm_act); FMR_act = zeros(n_act,1); ss_act = read_stat(nm_act, 's_s');
       PMR_act = groupDataMatrix.act(:,4); SMR_act = groupDataMatrix.act(:,3); T_act = groupDataMatrix.act(:,2); W_act = groupDataMatrix.act(:,1); 
      for i=1:n_act
        FMR_act(i) = 15.55 * get_FMR(nm_act{i}, W_act(i), T_act(i), 1); % ml O2/min
      end
      %
      nm_amp = groupClean.amp(:,4); n_amp = length(nm_amp); FMR_amp = zeros(n_amp,1); ss_amp = read_stat(nm_amp, 's_s');
       PMR_amp = groupDataMatrix.amp(:,4); SMR_amp = groupDataMatrix.amp(:,3); T_amp = groupDataMatrix.amp(:,2); W_amp = groupDataMatrix.amp(:,1); 
      for i=1:n_amp
        FMR_amp(i) = 15.55 * get_FMR(nm_amp{i}, W_amp(i), T_amp(i), 1); % ml O2/min
      end
      %
      nm_squ = groupClean.squ(:,4); n_squ = length(nm_squ); FMR_squ = zeros(n_squ,1); ss_squ = read_stat(nm_squ, 's_s');
      PMR_squ = groupDataMatrix.squ(:,4); SMR_squ = groupDataMatrix.squ(:,3); T_squ = groupDataMatrix.squ(:,2); W_squ = groupDataMatrix.squ(:,1); 
      for i=1:n_squ
        FMR_squ(i) = 15.55 * get_FMR(nm_squ{i}, W_squ(i), T_squ(i), 1); % ml O2/min
      end
      %
      nm_ave = groupClean.ave(:,4); n_ave = length(nm_ave); FMR_ave = zeros(n_ave,1); ss_ave = read_stat(nm_ave, 's_s');
       PMR_ave = groupDataMatrix.ave(:,4); SMR_ave = groupDataMatrix.ave(:,3); T_ave = groupDataMatrix.ave(:,2); W_ave = groupDataMatrix.ave(:,1); 
      for i=1:n_ave
        FMR_ave(i) = 15.55 * get_FMR(nm_ave{i}, W_ave(i), T_ave(i), 1); % ml O2/min
      end
      %
      nm_mar = groupClean.mar(:,4); n_mar = length(nm_mar); FMR_mar = zeros(n_mar,1); ss_mar = read_stat(nm_mar, 's_s');
      PMR_mar = groupDataMatrix.mar(:,4); SMR_mar = groupDataMatrix.mar(:,3); T_mar = groupDataMatrix.mar(:,2); W_mar = groupDataMatrix.mar(:,1); 
      for i=1:n_mar
        FMR_mar(i) = 15.55 * get_FMR(nm_mar{i}, W_mar(i), T_mar(i), 1); % ml O2/min
      end
      %
      nm_pla = groupClean.pla(:,4); n_pla = length(nm_pla); FMR_pla = zeros(n_pla,1); ss_pla = read_stat(nm_pla, 's_s');
       PMR_pla = groupDataMatrix.pla(:,4); SMR_pla = groupDataMatrix.pla(:,3); T_pla = groupDataMatrix.pla(:,2); W_pla = groupDataMatrix.pla(:,1); 
      for i=1:n_pla
        FMR_pla(i) = 15.55 * get_FMR(nm_pla{i}, W_pla(i), T_pla(i), 1); % ml O2/min
      end
      ...
      saveas(gcf,fullfile(outputDir,'ss_PFMR.fig'))
      saveas(gcf,fullfile(outputDir,'ss_PFMR.png'))
    ...
    case 5 % kap, s_s, kapRA
      ...
      %saveas(gca,fullfile(outputDir,'kap.png'))
      ...
      xlabel('supply stress s_s, -') 
      ylabel('survivor function')
      %saveas(gca,fullfile(outputDir,'ss.png'))
      ...
      ylabel('survivor function')
      %saveas(gca,fullfile(outputDir,'kapRA.png'))
   
    case 6 % simulation of standard DEB model with stochastic searching
      shtraject % description see DEBtool_M/animal/shtraject
      saveas(Hfig_O,fullfile(outputDir,'organics.png'))
      saveas(Hfig_M,fullfile(outputDir,'minerals.png'))
    ...
    case 7 % ss_var(j_Oi)
      ...
      saveas(gca,fullfile(outputDir,'ss_stdjTOi.png'))

      figure
      plot(ss, 1e3*std_jOi, 'ob')
      xlabel('supply stress, -')
      ylabel('standard dev spec ultimate resp, mmol O_2/d.g')
      saveas(gca,fullfile(outputDir,'ss_stdjOi.png'))
    ...
    case 11 % ss_jOi
      ...
      saveas(gca,fullfile(outputDir,'ss_jOi.png'))
  end
end
